...
const SpeedTestServerList = [
  { name: 'Server 1 (GitHub)', url: 'https://raw.githubusercontent.com/Kirakishou0Sukhoidau/Kirara/Kirara/video/Kama.mp4' }
 /* { name: 'Server 2 (Hetzner)', url: 'https://speed.hetzner.de/100MB.bin' },
  { name: 'Server 3 (ThinkBroadband)', url: 'http://ipv4.download.thinkbroadband.com/100MB.zip' },
  { name: 'Server 4 (OVH)', url: 'http://proof.ovh.net/files/100Mb.dat' },
  { name: 'Server 5 (Cachefly)', url: 'http://cachefly.cachefly.net/100mb.test' } */
];
...
function SpeedTestSetup(SpeedTestIndex, SpeedTestFileUrl) {
  ...
  SpeedTestStartBtn.onclick = () => {
    ...
    const SpeedTestSignal = SpeedTestController.signal;
    SpeedTestStatusDiv.textContent = 'Bắt đầu tải file...';

    SpeedTestStartTime = performance.now();
    SpeedTestStableReached = false;

    fetch(SpeedTestFileUrl + '?nocache=' + Date.now(), { cache: "no-store", signal: SpeedTestSignal })
      .then(response => {
        ...
        function SpeedTestRead() {
          return SpeedTestReader.read().then(({ done, value }) => {
            if (done) {
              ...
              SpeedTestStatusDiv.textContent += ' | Tải file hoàn tất!';
              return;
            }
            ...
            SpeedTestStatusDiv.innerText =
              ...
              `- Thời gian: ${SpeedTestElapsed.toFixed(1)}s\n` +
              `- Ước tính: ${SpeedTestEstimate.toFixed(1)}s`;
    ...
    function SpeedTestStartMeasure() {
      SpeedTestStatusDiv.textContent += ' | Bắt đầu Speedtest.js...';
    }

    function SpeedTestStop() {
